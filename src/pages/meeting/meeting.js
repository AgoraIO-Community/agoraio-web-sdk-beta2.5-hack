...
const optionsInit = () => {
  let options = {
    ...
    channel: Cookies.get('channel') || localStorage.getItem('channel')|| 'test',
...
const removeStream = id => {
  ...
  document.getElementById("volumeindicators").innerHTML="";
  
  streamList.map((item, index) => {   
    var element=document.createElement('div');
    element.setAttribute('background-color',"white");
    element.setAttribute("class","col-lg-"+Math.round(12/streamList.length))
  
    element.innerHTML= "User:-"+item.getId()+"Volume <div id="+item.getId()+"></div>";
    element.style.maxHeight = '100%';
    element.style.backgroundColor   ='white';
    element.style.border ="2px solid rgb(238, 97, 97)"

 
  document.getElementById("volumeindicators").appendChild(element);
  });
};
...
clientInit(client, options).then(uid => {
  ...
  enableDualStream();
  client.getRecordingDevices(function(){
    ...
    () => {
     ...
     client.publish(localStream,err => {
      ...
      setInterval(function () {
        client.getLocalVideoStats(function(stats){
          console.log(stats)
document.getElementById('localvideo').innerText= stats[uid][document.getElementById('videoselector').value] 
  }   )     
      ...
      setInterval(function () {
        client.getLocalAudioStats(function(stats){
document.getElementById('localaudio').innerText= stats[uid][document.getElementById('audioselector').value]   }   )     
                 
      },100)

      setInterval(function () {
        client.getNetworkStats(function(stats){
          document.getElementById('network').innerText=stats.NetworkType
                 })
      ...
      setInterval(function () {
        client.getSystemStats(function(stats){
          document.getElementById('batlevel').innerText=stats.BatteryLevel;
                 })
      ...
      setInterval(function(){     
        ...
        streamList.map((item, index) => {  
          document.getElementById(item.getId()).style.width =(item.getAudioLevel()*100)+'%';
          document.getElementById(item.getId()).style.height='10px';
                    document.getElementById(item.getId()).style.height='10px';


          document.getElementById(item.getId()).style.backgroundColor='green' ;
        }) }, 1);
